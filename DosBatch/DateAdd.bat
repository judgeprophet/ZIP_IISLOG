...
SET _FIND=C:\WINDOWS\SYSTEM32\FIND.EXE
...
FOR /F "tokens=1* delims==" %%A IN ('TYPE "%TEMP%.\_TEMP.REG" ^| %_FIND% /I "iDate"') DO SET iDate=%%B
FOR /F "tokens=1* delims==" %%A IN ('TYPE "%TEMP%.\_TEMP.REG" ^| %_FIND% /I "sDate"') DO SET sDate=%%B
